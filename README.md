...
> [!WARNING]
> **Beta software:** BetterVoice 0.1 is an early beta for Windows 10 and 11. Expect rough edges, and report problems through [GitHub Issues](https://github.com/kerim0x1/bettervoice/issues).

<p align="center">
  Press <kbd>Win</kbd>+<kbd>O</kbd>, speak, press it again — your words appear wherever your cursor is.<br>
  Transcribed on your own PC, or by the cloud engine you choose.
</p>
...
**BetterVoice is a dictation app for Windows.** It works in every program that accepts text: your editor, your browser, your chat, your mail. There is no window to switch to and nothing to copy — BetterVoice listens while you speak and types the result where you were already working.

Pick how your speech is recognized. Keep everything on your PC with the built-in offline model, or connect a cloud engine for live results and extra accuracy.

## Four ways to listen

| Engine | How it works | You need |
| --- | --- | --- |
| **Local** | OpenAI's Whisper runs on your PC, fully offline after a one-time model download. Uses your NVIDIA graphics card when there is one. | Nothing |
...
- **Private when you want it.** With the offline engine, audio never leaves your PC. BetterVoice never writes what you dictate to its logs.
- **Polished on request.** The optional *AI Polish* removes filler words like "um" and "uh" and small slips from any engine's result.
- **Set up in a minute.** A short first-start setup walks you through engine, key or model download, and language. Everything can be changed later from the tray icon.

![The overlay: listening, transcribing, and a short message](assets/screenshots/overlay.png)

## Download

BetterVoice is free and open source, for Windows 10 and 11, 64-bit. Download it from the [GitHub Releases page](https://github.com/kerim0x1/bettervoice/releases): the newest release lists its files under **Assets**.

| File | Choose it for |
| --- | --- |
| `BetterVoice-<version>-win-x64-setup.exe` | **Most PCs.** Installs BetterVoice for your Windows account; no administrator rights needed. |
| `BetterVoice-<version>-win-x64-cuda-setup.exe` | PCs with an NVIDIA graphics card: the offline engine runs on the GPU. A larger download. |
| `BetterVoice-<version>-win-x64.zip` and `…-cuda.zip` | Using BetterVoice without installing it: unzip, then start `BetterVoice.exe`. |

Every engine works with every file; the CUDA edition only adds GPU support for the offline engine. BetterVoice is not code-signed yet: if SmartScreen warns about an unrecognized app, choose *More info → Run anyway*.

## Start here

1. [Download](#download) the installer and run it.
2. Start BetterVoice from the Start menu, or let the installer start it. The setup opens on the first start.
3. Choose an engine, then enter its key or download the offline model.
4. Click into any text field, press <kbd>Win</kbd>+<kbd>O</kbd>, speak, and press <kbd>Win</kbd>+<kbd>O</kbd> again.
...
<p align="center"><strong>BetterVoice</strong><br>Your voice, typed anywhere.<br><sub>Beta · Windows 10 &amp; 11 · MIT License · Built by <a href="https://kerim0x1.com">kerim0x1</a> · Also from the maker: <a href="https://betterc0de.com">BetterC0de</a></sub></p>
